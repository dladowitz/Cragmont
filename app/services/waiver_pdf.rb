...
class WaiverPdf
  ...
  def initialize(waiver:, signature_png:)
    @waiver = waiver
  ...
  def render
    Prawn::Document.new(page_size: "LETTER", margin: 54) do |pdf|
      ...
      detail(pdf, "Participant", user.full_name)
      detail(pdf, "Email", user.email.presence || "None")
      detail(pdf, "Waiver year", @waiver.waiver_year)
      detail(pdf, "Waiver type", @waiver.display_type)
      render_trip_details(pdf)
      render_minors(pdf)
      divider(pdf)
      detail(pdf, "Plain Text Acknowledged at", formatted_metadata_time(@waiver.waiver_acknowledged_at))
      detail(pdf, "Acknowledgement digest", @waiver.waiver_acknowledgement_text_digest)
  ...
  def signup
    @waiver.campsite_signup
  end

  def trip
    @waiver.trip || signup&.trip
  end
  ...
  def render_waiver_metadata(pdf)
    ...
    divider(pdf)
    detail(pdf, "Waiver Signed at", formatted_metadata_time(@waiver.waiver_signed_at))
    detail(pdf, "Signature digest", @waiver.waiver_signature_digest)
    detail(pdf, "Waiver digest", @waiver.waiver_text_digest)
  end
  ...
  def acknowledgement_blocks
    text = @waiver.waiver_acknowledgement_text.presence || TripSignupWaiver.acknowledgement_text(includes_minors: includes_minors?)
    TripSignupWaiver.blocks_from(text)
  end

  def waiver_blocks
    text = @waiver.waiver_text.presence || TripSignupWaiver.text(includes_minors: includes_minors?)
    TripSignupWaiver.blocks_from(text)
  end
end
