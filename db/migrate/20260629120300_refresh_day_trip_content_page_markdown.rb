class RefreshDayTripContentPageMarkdown < ActiveRecord::Migration[8.1]
  ...
  def up
    execute <<~SQL
      ...
      SET body = #{quote(SAFETY_BODY)}, updated_at = CURRENT_TIMESTAMP
      WHERE slug = 'how_to_think_about_safety'
    SQL
    ...
    execute <<~SQL
      ...
      WHERE slug = 'what_to_expect'
        AND body NOT LIKE '%## Day trips%'
    SQL
  end

  def down
    # Content pages are user-editable, so do not rewrite them on rollback.
  end
end
